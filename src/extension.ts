...
export async function activate(context: vscode.ExtensionContext) {
	...
	if (!snapshotTimer) {
		snapshotTimer = setInterval(async () => {
			try { const txt = await getChatInputText(); if (txt && txt !== typingBuffer) { lastSnapshot = txt; } } catch {}
		}, 1200);
	...
	const handleForwardEnter = async (variant: string) => {
		try {
			...
			if (!ok) {
				for (const id of ['github.copilot.chat.acceptInput','github.copilot.chat.send','github.copilot.chat.submit','github.copilot.interactive.submit','workbench.action.chat.acceptInput','workbench.action.chat.submit','workbench.action.chatEditor.acceptInput']) { try { await vscode.commands.executeCommand(id); ok = true; break; } catch {} }
			}
