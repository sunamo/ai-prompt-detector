...
export class PromptsProvider implements vscode.WebviewViewProvider {
	...
	constructor(private recentPrompts: string[]) {
	...
	public resolveWebviewView(
		...
		writeLog(`🎯 PROMPTS: Number of prompts to display: ${this.recentPrompts.length}`, false);
	}
	...
	private updateWebview(): void {
		...
		writeLog(`🎯 PROMPTS: HTML set, displaying ${this.recentPrompts.length} prompts`, true);
	}
	...
	private createPromptsHtml(): string {
		...
		if (this.recentPrompts.length > 0) {
			const displayPrompts = this.recentPrompts.slice(0, maxPrompts);
...
<div class="header-bar">
	📊 Total: ${this.recentPrompts.length} prompts (max ${maxPrompts}) | ⚙️ Change max count in settings
</div>
